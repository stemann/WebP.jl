...
function WebPConfigInit(config)
    return WebPConfigInitInternal(config, WEBP_PRESET_DEFAULT, Cfloat(75), WEBP_ENCODER_ABI_VERSION)
end
...
function WebPIDecGetYUV(idec, last_y, u, v, width, height, stride, uv_stride)
    return WebPIDecGetYUVA(idec, last_y, u, v, C_NULL, width, height, stride, uv_stride, C_NULL)
end
...
function WebPIsAlphaMode(mode)
    return Cint(mode == MODE_RGBA || (mode == MODE_BGRA || (mode == MODE_ARGB || (mode == MODE_RGBA_4444 || (mode == MODE_YUVA || WebPIsPremultipliedMode(mode) != 0)))))
end

function WebPIsPremultipliedMode(mode)
    return Cint(mode == MODE_rgbA || (mode == MODE_bgrA || (mode == MODE_Argb || mode == MODE_rgbA_4444)))
end
